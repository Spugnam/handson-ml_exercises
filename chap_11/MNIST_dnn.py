...
if __name__ == "__main__":
    ...
    reset_graph()
    n_inputs = 28*28
    n_hidden = 100  # for hidden layers 1 to 5
    n_outputs = 5
    # training
    learning_rate = 0.01
    n_epochs = 30
    ...
    with tf.name_scope("DNN"):
        ...
        hidden1 = my_dense_layer(X_drop, n_hidden, name="hidden1")
        hidden1_drop = my_dropout_layer(hidden1)
        bn1 = tf.nn.elu(my_batch_norm_layer(hidden1_drop))
        hidden2 = my_dense_layer(bn1, n_hidden, name="hidden2")
        ...
        hidden4 = my_dense_layer(bn3, n_hidden, name="hidden4")
        hidden4_drop = my_dropout_layer(hidden4)
        bn4 = tf.nn.elu(my_batch_norm_layer(hidden4_drop))
        hidden5 = my_dense_layer(bn4, n_hidden, name="hidden5")
        hidden5_drop = my_dropout_layer(hidden5)
        bn5 = tf.nn.elu(my_batch_norm_layer(hidden5_drop))
    ...
    with tf.Session() as sess:
        ...
        for epoch in range(start_epoch, n_epochs):
            ...
            acc_train = accuracy.eval(feed_dict={X: X_train_0_4,
                                                 y: y_train_0_4})
            acc_val = accuracy.eval(feed_dict={X: X_test_0_4, y: y_test_0_4})
